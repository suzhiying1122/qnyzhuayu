...
function updateHash(viewName) {
  const nextHash = getHashForView(viewName);
...
function getHashForView(viewName) {
  ...
  if (viewName === "postDetail") return `#post/${encodeURIComponent(state.activePostId || "")}`;
  if (viewName === "activityDetail") return `#activity/${encodeURIComponent(state.activeActivityId || "")}`;
  if (viewName === "letterDetail") return `#letter/${encodeURIComponent(state.activeLetterId || "")}`;
  return `#${viewName}`;
}
...
async function handlePostSubmit(event) {
  ...
  state.pendingPosts.unshift({
    id: createId("pending-post"),
    title,
    body,
  ...
  try {
    ...
    state.pendingPosts.shift();
    showToast("浏览器本地空间不足，附件未保存");
    return;
  }
...
function approvePost(id) {
  if (!requireAdminKey()) return;
  const index = state.pendingPosts.findIndex((item) => item.id === id);
  if (index < 0) return;
  const pending = state.pendingPosts.splice(index, 1)[0];
  const post = {
    ...pending,
    id: createId("post"),
...
function approveActivity(id) {
  if (!requireAdminKey()) return;
  const index = state.pendingActivities.findIndex((item) => item.id === id);
  if (index < 0) return;
  const pending = state.pendingActivities.splice(index, 1)[0];
...
function rejectPending(type, id) {
  if (!requireAdminKey()) return;
  const collections = {
    post: state.pendingPosts,
    comment: state.pendingComments,
    activity: state.pendingActivities,
  };
...
function bindPostDetailForms() {
  elements.postDetailContent.querySelectorAll("[data-comment-form]").forEach((form) => {
    form.addEventListener("submit", (event) => {
      ...
      if (!body) return;
      addCommentToPost(postId, body, parentId);
...
function addCommentToPost(postId, body, parentId = "") {
