...
- 采用用户选定的 C 版构图：暖白文字区与右侧整幅剧场图；根据后续“增加隐纹与巧思”的反馈，加入剧场座席弧线暗纹、错位标题和票根式入口，悬停时票根轻微移动、舞台亮度变化。
- 当前为本地设计预览，尚未覆盖生产站点，等待用户对实装效果的反馈。
- 新舞台素材为装饰性生成图片，WebP 约 196 KB，替代此页原先加载的两张约 3.7 MB PNG。旧素材文件保留。
- 入场过渡缩短为 420ms，增加背景 inert、键盘焦点限制与入场后焦点恢复；保留会话内跳过和减少动态效果偏好，存储被禁用时仍可进入。
- 验证生产构建、1440/390/320px、键盘入场、刷新跳过、减少动态效果、sessionStorage 不可用与入场后的导航。本次无数据库改动。

## 2026-09-09 投稿文章阅读页

- 文章详情与新版个人空间采用一致的暖白纸面、深色正文和酒红操作色；移除该页旧深绿场景与嵌套阅读卡片。
- 保留文章标题、作者、发布时间、原文换行、附件和征文说明；正文限制在 720px 阅读栏，手机端自适应。
- 保留返回征文及管理员删除确认，缺失文章提供明确返回入口。此更新仅涉及前端，不需要 D1 迁移。
- 验证生产构建、1440/390/320px 浏览器布局、正文一致性、返回入口、管理员与访客权限显示。
